<?
class ELST extends ComponentBlock_ComponentBlock implements impComponentBlock{

	// 초기화
	public function ELST($inputdata,$isString,$fieldName,$formExtraValue){
	...
	public function blockList(){
		...
		if($ctn_complete == 1 && !$arrSplitData[0]){
			$retdate=explode("-",$row[$this->arr_field['처리기한'][0]]);
			$d1=mktime(0,0,0,$retdate[1],$retdate[2],$retdate[0]);
			$d2=mktime(0,0,0,date('m'),date('d'),date('Y'));
			$restday=(($d1-$d2)/86400);
			if($restday>0){
				$result_form .= "<b><font style='color:brown'>회신결과 입력X</font><br><font style='color:gray'>(".(($d1-$d2)/86400)."일 남았음)</font></b>";
			}else{
				$result_form .= "<b><font style='color:brown'>회신결과 입력X</font><br><font style='color:red'>(".(($d1-$d2)/86400)."일 지났음)</font></b>";
			}
