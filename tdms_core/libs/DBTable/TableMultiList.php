...
class DBTable_TableMultiList extends DBTable_TableDataList{
	...
	private function FirstDataSetting($tbNameValue){
		if($this->TableType == true){ //여러개일 때
			...
			$this->FirstData[2] = $this->arrMediumCategory[$tbNameValue][2];	// 테이블의 필드  제목명(배열)($arr_tb_item)
			$this->FirstData[3] = $this->arrMediumCategory[$tbNameValue][3];	// db테이블의 필드명(배열)($arr_tb_field)
	...
	public function funcListViewTable($dsnval, $tbname,  $subtitle, $isarr){
		...
		//$this->setBasicInputQuery();  //쿼리를 구성
		...
		if($this->likeis){  // (특수한 자료만 있는 경우에 검색하지 않음) => 주민번호가 없는 자료는 검색하지 않는다.
			...
			if($rowvalue <= 0){ // 값이 없는경우, 출력하지 않는다.
				array_push($this->arrnoSearchCheck, $this->title);
			}
			else if(($isarr==true && $rowvalue > 0) || $isarr == false){ // 배열 dsn이고 값이 있는 경우 출력
